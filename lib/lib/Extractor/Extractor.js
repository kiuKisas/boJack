      ...
      .then(queries => {
        const promisesByVariables = outputConfig.arrayVariables.map(async variables => {
          ...
          if (outputConfig.type === 'gql') {
            promisesQueries = queries.map(async query => {
            ...
            console.log(this.url + variables.url)
            promisesQueries = [fetch(this.url + variables.url, {method: 'GET'}).then(res => res.json()).then(async json => {
              const data = boExtractHelpers.callbacksByKeys(json, outputConfig.callbacks.byQueries)
              const doRec = Object.keys(innerCallback).length > 0
              console.log(doRec)
              return doRec ? await recDeeper(innerCallback, data) : data
            })]
          ...
          return Promise.all(promisesQueries).then(dataArrays => {
            ...
            if (outputConfig.payload.save.includes('variables')) {
              ...
              return Promise.all(splitted.map(async split => {
                return boExtractHelpers
                  .saveFile(split.data, outputConfig.payload, pathBases.payload, split.variables)
                  .then(res => {
        ...
        console.log(promisesByVariables)
