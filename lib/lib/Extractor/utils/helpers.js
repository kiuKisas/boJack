...
const boExtractHelpers = {
  ...
  saveFile(data, dataPayload, pathBase, variables = {}) {
    ...
    const filename = dataPayload.callbacks.name(dataPayload, variables, data)
    const directory = dataPayload.callbacks.directory(dataPayload, variables, data)
    const payloadPath = path.join(pathBase, directory, filename)
  ...
  async callbacksByKeysEnsure(data, callbacks, ensureCallback) {
    // Your callbacks is your responsability.
    const keys = Object.keys(data)
    const cbKeys = Object.keys(callbacks)
    const dataCallbacked = {}
    for (let key of keys) {
      if (cbKeys.includes(key)) {
        const value = await callbacks[key](data[key])
        dataCallbacked[key] = value
      } else {
        const tmp = await ensureCallback(data[key])
        dataCallbacked[key] = tmp
      }
    }
    return Object.assign({}, data, dataCallbacked)
  },
