...
function makeCallback(config, pathBase) {
  ...
  // TODO: should be async/ promise
  return url => {
    const ext = url.split('.').pop()
    if (!fileExtensions.includes(ext)) return url
    const assetPath = path.join(filePaths[ext], config.directory, parsePath(url))
    const newPath = newValue(url, assetPath)
    if (preventDownload(assetPath)) return newPath
